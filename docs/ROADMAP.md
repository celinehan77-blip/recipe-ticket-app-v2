...
└── Phase B：Platform Coverage（进行中）
    └── Checkpoint B1：抖音公开视频 MVP（进行中）
        ...
        ├── Task：TikHub 公开媒体适配与 FFmpeg 复用（已完成）
        ├── Task：5 条视频真实端到端验收（等待服务端 Key）
...
- Current Version：`0.2.0-working.13`
...
Checkpoint B1 进行中：抖音短链会经过严格域名校验，使用 TikHub App V3 解析公开作品媒体，再复用现有 `FFmpeg -> 火山 ASR -> DeepSeek -> Supabase` 单一路径。已确认直接 `yt-dlp` 对真实抖音样本要求新鲜 Cookie，不适合作为 Vercel 生产依赖；实现和 76 项本地无付费测试已通过，等待配置服务端 `TIKHUB_API_KEY` 后对 5 条视频和 1 条图文样本各调用一次。图文样本无音轨时当前安全返回 OCR 尚未支持，不生成虚假菜谱。
...
| 4 | 抖音链接解析 | 进行中 | TikHub 服务端 Key 与 6 条真实样本验收 |
