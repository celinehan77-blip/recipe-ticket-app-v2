...
→ URL 规范化
→ 平台域名与短链检查
→ 小红书使用 yt-dlp；抖音使用 TikHub App V3 解析公开媒体
...
- 当前媒体链路支持小红书与抖音公开视频：小红书使用 `yt-dlp`，抖音使用 TikHub App V3。
...
1. 小红书由 `yt-dlp` 读取公开元数据；抖音由 TikHub 返回公开作品媒体数据，并检查 5 分钟、100 MB 限制。
...
抖音适配器和无付费测试已完成，真实样本验收等待配置服务端 TikHub Key。抖音图文作品当前无音轨时返回安全错误，OCR 留到后续 Checkpoint。
